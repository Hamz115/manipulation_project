...
void setTarget(const std::string &name, geometry_msgs::msg::Pose &pose, const std::vector<double> &target, const rclcpp::Logger &logger) {
...
void pick_and_place(float x, float y) {
    // Node configuration
    ...
    const std::vector<double> PRE_GRASP_POSE{x, y, 0.3, -1.0, 0.00, 0.00, 0.00};
    const std::vector<double> GRASP_POSE{x, y, 0.17, -1.0, 0.00, 0.00, 0.00};
    // Planning interfaces
    ...
    // Open gripper 
    RCLCPP_INFO(move_group_node->get_logger(), "Opening gripper.");
    ...
    if (success_gripper) {
        RCLCPP_INFO(move_group_node->get_logger(), "Plan to actuate gipper: SUCCESS.");
        RCLCPP_INFO(move_group_node->get_logger(), "Executing command.");
        move_group_arm.execute(plan_gripper);
    } else {
        RCLCPP_INFO(move_group_node->get_logger(), "Plan to actuate gipper: FAIL.");
        RCLCPP_INFO(move_group_node->get_logger(), "Aborting command.");
        rclcpp::shutdown();
        return;
    }

    // Pre-grasp position
    setTarget("pre_grasp", target_pose, PRE_GRASP_POSE, move_group_node->get_logger());
    move_group_arm.setPoseTarget(target_pose);
    success_arm = (move_group_arm.plan(plan_arm) == moveit::core::MoveItErrorCode::SUCCESS);
    if (success_arm) {
        RCLCPP_INFO(move_group_node->get_logger(), "Plan to target status: SUCCESS.");
        RCLCPP_INFO(move_group_node->get_logger(), "Executing motion.");
        move_group_arm.execute(plan_arm);
    } else {
        RCLCPP_INFO(move_group_node->get_logger(), "Plan to target status: FAIL.");
        RCLCPP_INFO(move_group_node->get_logger(), "Aborting motion.");
        rclcpp::shutdown();
        return;
    }
    ...
    setTarget("grasp", target_pose, GRASP_POSE, move_group_node->get_logger());
    grasp_waypoints.push_back(target_pose);
    ...
    RCLCPP_INFO(move_group_node->get_logger(), "Fraction: %.3f.", float(fraction));
    move_group_arm.execute(trajectory_approach);

    // Add a short delay before closing the gripper to ensure the object is in position
    ...
    RCLCPP_INFO(move_group_node->get_logger(), "Closing gripper: %.3f.", gripper_value);
    success_gripper = (move_group_gripper.plan(plan_gripper) == moveit::core::MoveItErrorCode::SUCCESS);
    if (success_gripper) {
        RCLCPP_INFO(move_group_node->get_logger(), "Plan to actuate gripper: SUCCESS.");
        RCLCPP_INFO(move_group_node->get_logger(), "Executing command.");
        move_group_arm.execute(plan_gripper);
        gripper_value += 0.001;
        // Add a small delay between each increment
        std::this_thread::sleep_for(std::chrono::milliseconds(50000));
    } else {
        RCLCPP_INFO(move_group_node->get_logger(), "Plan to actuate gripper: FAIL.");
        RCLCPP_INFO(move_group_node->get_logger(), "Aborting command.");
        rclcpp::shutdown();
        return;
    }}
    ...
    success_arm = (move_group_arm.plan(plan_arm) == moveit::core::MoveItErrorCode::SUCCESS);
    if (success_arm) {
        RCLCPP_INFO(move_group_node->get_logger(), "Plan to rotate first joint 180 degrees: SUCCESS.");
        RCLCPP_INFO(move_group_node->get_logger(), "Executing rotation.");
        move_group_arm.execute(plan_arm);
    } else {
        RCLCPP_INFO(move_group_node->get_logger(), "Plan to rotate first joint 180 degrees: FAIL.");
        RCLCPP_INFO(move_group_node->get_logger(), "Aborting rotation.");
        rclcpp::shutdown();
        return;
    }
...
int main(int argc, char **argv) {
    ...
    pick_and_place(action_client->get_x(), action_client->get_y());
